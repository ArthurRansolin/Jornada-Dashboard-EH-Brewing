...
PROFILE_DEFS = [
    {
        'name': 'Ale Padrão',
        'description': 'Fermentação ale com descanso de diacetil e cold crash curto.',
        'segments': [(1, 18.0, 48), (2, 20.0, 24), (3, 4.0, 36)],
    },
    {
        'name': 'Lager Clássica',
        'description': 'Fermentação fria, descanso de diacetil e maturação longa.',
        'segments': [(1, 10.0, 120), (2, 15.0, 36), (3, 2.0, 120)],
    },
    {
        'name': 'Saison Expressiva',
        'description': 'Rampa crescente para esterificação controlada de levedura saison.',
        'segments': [(1, 19.0, 24), (2, 23.0, 36), (3, 26.0, 48), (4, 3.0, 24)],
    },
    {
        'name': 'Stout Limpa',
        'description': 'Perfil robusto de ale escura com estabilização final.',
        'segments': [(1, 17.5, 72), (2, 19.5, 24), (3, 5.0, 48)],
    },
    {
        'name': 'Kolsch Delicada',
        'description': 'Fermentação limpa com maturação fria curta para cervejas claras.',
        'segments': [(1, 16.0, 72), (2, 18.0, 24), (3, 3.0, 72)],
    },
    {
        'name': 'Belgian Alta',
        'description': 'Rampa quente para favorecer esterificação em leveduras belgas.',
        'segments': [(1, 18.5, 24), (2, 21.0, 36), (3, 24.0, 48), (4, 4.0, 48)],
    },
    {
        'name': 'Weiss Frutada',
        'description': 'Perfil para trigo com inicio moderado e final mais aromático.',
        'segments': [(1, 17.0, 36), (2, 20.0, 48), (3, 3.0, 36)],
    },
    {
        'name': 'Kveik Quente',
        'description': 'Fermentação rápida em alta temperatura para leveduras kveik.',
        'segments': [(1, 30.0, 36), (2, 32.0, 24), (3, 5.0, 24)],
    },
]

BEER_TYPE_DEFS = [
    ('American IPA', 'Ale Padrão', 18, 20, 'Base lupulada com fermentação limpa e final seco.'),
    ('West Coast IPA', 'Ale Padrão', 18, 20, 'IPA resinosa com perfil limpo para destacar lúpulo.'),
    ('New England IPA', 'Ale Padrão', 18, 21, 'IPA turva com fermentação levemente mais quente.'),
    ('Pilsen', 'Lager Clássica', 9, 12, 'Lager clara com maturação fria e alta limpidez.'),
    ('Helles', 'Lager Clássica', 9, 12, 'Lager maltada, delicada e de fermentação fria.'),
    ('Saison', 'Saison Expressiva', 19, 26, 'Cerveja esterificada com rampa crescente.'),
    ('Dry Stout', 'Stout Limpa', 17, 20, 'Ale escura seca com perfil estável.'),
    ('Porter', 'Stout Limpa', 17, 20, 'Ale escura maltada com finalização fria curta.'),
    ('Witbier', 'Weiss Frutada', 17, 21, 'Trigo belga leve, aromático e refrescante.'),
    ('Weissbier', 'Weiss Frutada', 17, 22, 'Trigo alemão com foco em ester e fenol.'),
    ('Kolsch', 'Kolsch Delicada', 15, 18, 'Cerveja clara hibrida com maturação fria curta.'),
    ('Belgian Blond', 'Belgian Alta', 18, 24, 'Belga clara com rampa quente e final aromático.'),
    ('Kveik Pale Ale', 'Kveik Quente', 28, 34, 'Ale rápida com fermentação quente e perfil tropical.'),
]
...
TANK_DEFS = [
    ...
    ('Tanque Piloto', 150, 'Bancada piloto', 'manutenção', None, 24),
    ('Tanque Barril', 300, 'Câmara fria', 'idle', None, 25),
]

BATCH_DEFS = [
    ('IPA Citrus da Casa', 'Fermentador F01', 'American IPA', 'Ale Padrão', 'running', 2, None, 'US-05', 1.060, 1.012, 3),
    ('Pilsen Clara', 'Fermentador F02', 'Pilsen', 'Lager Clássica', 'running', 6, None, 'W-34/70', 1.048, 1.010, 11),
    ...
    ('NEIPA Manga', 'Fermentador F06', 'New England IPA', 'Ale Padrão', 'running', 1, None, 'Verdant IPA', 1.066, 1.014, 37),
    ...
    ('Helles Maltada', 'Fermentador F10', 'Helles', 'Lager Clássica', 'running', 8, None, 'S-189', 1.047, 1.009, 53),
    ('West Coast Lote 18', 'Fermentador F01', 'West Coast IPA', 'Ale Padrão', 'finished', 22, 7, 'US-05', 1.062, 1.011, 61),
    ('Pilsen Safra 12', 'Fermentador F02', 'Pilsen', 'Lager Clássica', 'finished', 40, 16, 'W-34/70', 1.046, 1.009, 67),
    ...
    ('Helles Jardim', 'Fermentador F10', 'Helles', 'Lager Clássica', 'finished', 55, 18, 'S-189', 1.048, 1.010, 107),
]
...
def create_profile_commands(db, batch, tank, profile, start_at):
    ...
    db.add(CommandLog(
        ...
        value_sent='1',
        success=True,
        response_text='Controlador colocado em automático.',
...
def create_alarms(db, tanks):
    now = utcnow_naive().replace(microsecond=0)
    alarm_defs = [
        ('Fermentador F01', now - timedelta(hours=7), now - timedelta(hours=6, minutes=20), 'warning', 'temperature_deviation', 'PV ficou acima do SP durante a estabilização inicial.', 'status_word_1=1'),
        ...
        ('Tanque Reserva 01', now - timedelta(days=1, hours=2), now - timedelta(days=1, hours=1, minutes=30), 'info', 'manual_mode', 'Controlador reserva deixado em manual durante higienização.', 'run_state=0'),
    ]
